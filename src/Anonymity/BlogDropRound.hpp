...
namespace BlogDropPrivate {
  ...
  class BlogDropRound : public BaseBulkRound
  {
    ...
    public:
      ...
      void Resume(int idx)
      {
        const int slot_length = 6;
        m_resumed = true;
        _state->slots_open = QBitArray(GetGroup().Count(), false);
        _state->slots_open[idx] = true;
        _state->blogdrop_clients[idx]->GetParameters()->SetNElements(slot_length);

        if(IsServer()) {
          _server_state->blogdrop_servers[idx]->GetParameters()->SetNElements(slot_length);
        }
